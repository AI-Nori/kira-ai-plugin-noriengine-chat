...
is 50 (question 15 + request 20 + long text 15), so the cap is chosen as
``threshold 80 - 50 = 30``: flooding can only raise the backlog to 30, and
an ordinary group message (no @/mention) only exactly reaches the default
threshold 80 in the extreme combination of fully maxed content signals plus
a topped-out backlog (still further reduced by presence suppression and the
pace multiplier). With the backlog normalization floor of 3 (see
``noriengine_session_state._BACKLOG_NORM_FLOOR``), an isolated single message
gets only 6 backlog points (50+6=56 < 80), so the threshold-reaching
combination requires a real burst of ≥3 messages within 5 seconds.
"""
...
class TriggerVerdict:
    ...
    ``score`` serves both the strong-signal threshold check and the
    weak-signal accumulation view (the accumulation side applies the
...
def evaluate_trigger_score(
    ...
    raw = relevance + content_points + pressure - suppression
    ...
    return TriggerVerdict(score=final, breakdown=" ".join(parts))
...
def _content_points(
    ...
    *,
    ...
    if is_low_content_batch(cleaned_texts, low_content):
        points -= 25
        reasons.append("低价值")
